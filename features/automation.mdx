...
**Storage:** Jobs persist to `~/.openclaw/cron/jobs.json`
...
Check if enabled:
```bash
openclaw cron get <job-id> | grep enabled
```

Check schedule errors:
```bash
openclaw cron get <job-id> | grep scheduleErrorCount
```
...
```bash
openclaw cron get <job-id> | grep lastDeliveryStatus
```
