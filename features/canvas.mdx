...
Canvas Host is a local HTTP server that serves interactive HTML/JS/CSS from `~/.openclaw/state/canvas/` and provides:
...
Default canvas location: `~/.openclaw/state/canvas/`
...
4. **Check logs:**
   ```bash
   tail -f ~/.openclaw/logs/gateway.log | grep canvas
   ```
...
```bash
ls ~/.openclaw/state/canvas/index.html
```
